...
patches, texts, autotexts = ax.pie(pkg_tx_num_map.values(), labels=None, autopct='%1.1f\%%', startangle=0, pctdistance=0.8, colors=COLORS)
...
for i, patch in enumerate(patches):
    if float(autotexts[i].get_text()[:-3]) < 5.0:
        ...
        ax.annotate(autotexts[i].get_text(), xy=(x, y), xytext=(0.6*np.sign(x) + 0.3*k, (1.25 - 0.05*k)*y),
...
plt.savefig('./../results/workspace1/pkg_pie.pdf', format='pdf')

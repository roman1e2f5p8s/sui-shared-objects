...
patches, texts, autotexts = ax.pie(obj_tx_num_map2.values(), labels=None, autopct='%1.1f\%%', startangle=40, pctdistance=0.8, colors=COLORS)
...
for i, patch in enumerate(patches):
    if float(autotexts[i].get_text()[:-3]) < 5.0:
        ...
        if k == 0:
            ax.annotate(autotexts[i].get_text(), xy=(x, y), xytext=(-0.9*np.sign(x) + 0.32*k, (1.05 + 0.17*k)*y),
                    horizontalalignment=horizontalalignment, **kw)
        elif k < 3:
            ax.annotate(autotexts[i].get_text(), xy=(x, y), xytext=(-0.9*np.sign(x) + 0.32*k, (1.015 + 0.17*k)*y),
                    horizontalalignment=horizontalalignment, **kw)
        elif k < 4:
            ax.annotate(autotexts[i].get_text(), xy=(x, y), xytext=(-0.345*np.sign(x) + 0.32*k, (1.00 + 0.15*k)*y),
                    horizontalalignment=horizontalalignment, **kw)
        elif k < 7:
...
fig.tight_layout()
ax.set_position([-0.19, 0.04, 1.0, 1.0])
plt.savefig('./../results/workspace1/obj_pie.pdf', format='pdf')
